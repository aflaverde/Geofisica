...
##Correción para los datos
f_interp = InterpolatedUnivariateSpline(x_domo, g_domo, k=3)
x_2 = linspace(0, 6000)
y_interp = f_interp(x_2)
plt.plot(x_2, y_interp, label='Interpolación de datos')
plt.plot(x_domo, g_domo, 'x', mew=3, label='Datos domo')
plt.title("Anomalía gravitacional de un domo de sal")
plt.legend()

mitad2 = min(y_interp)/2 #Minimo medio de la anomalía interpolada
...
prof_real = dist_med2 * coef1 #Arroja la profundidad del domo real usando la interpolación
prof_real
...
def radio_domo(z, x, p1, p2): #p1: densidad de la sal, p2: densidad del subsuelo
    y = ((3*mitad2*(x**2 + z**2)) / (4*pi*G*(p1-p2)*z))**(0.33333)
    return y

p_sal = 2200
print(radio_domo(prof_real, half2[1], p_sal, roS))
...
plt.plot(x[1:], der_2, label='p_e=3000, h variable')

der_3 = np.diff(anom_3)/np.diff(x)

##z variable
der_4 = np.diff(anom_4)/np.diff(x)
plt.plot(x[1:], der_4, label='p_e=3000, z variable')
...
plt.plot(x[2:], fprime2, label='p_e=3000, h variable')

fprime3 = (der_3[1:] - der_3[:(len(der_3)-1)])/(x[1]-x[0])

##z variable
fprime4 = (der_4[1:] - der_4[:(len(der_4)-1)])/(x[1]-x[0])
plt.plot(x[2:], fprime4, label='p_e=3000, z variable')
